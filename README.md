...
- Throughout the repository, the name `spot_micro` only serves as an example and we provide a script to rename all the references to it automatically:

```bash
# Enter the repository
cd IsaacLabExtensionTemplate
# Rename all occurrences of spot_micro (in files/directories) to your_fancy_extension_name
...
```bash
python -m pip install -e source/spot_micro
```
...
We provide an example UI extension that will load upon enabling your extension defined in `source/spot_micro/spot_micro/ui_extension_example.py`.
...
```json
{
    "python.analysis.extraPaths": [
        "<path-to-ext-repo>/source/spot_micro"
    ]
}
```
